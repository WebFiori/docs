...
* [The Class `FileUploader`](#the-class-uploader)
* [Using the Class `FileUploader`](#using-the-class-uploader)
...
The framework has a utility class that can be used to handle file uploads using minimum amount of effort and code. The name of the class is [`FileUploader`](https://webfiori.com/docs/WebFiori/File/FileUploader) and its part of the namespace `WebFiori\File`.

## The Class `FileUploader`

The class [`FileUploader`](https://webfiori.com/docs/WebFiori/File/FileUploader) is a utility class which is used to handle file uploads in PHP in a simple way. It has all necessary tools to handle one file upload or multiple files upload. The class will help in achieving the following:
...
* In the back-end:
  ...
  * In the API, create new instance of the class [`FileUploader`](https://webfiori.com/docs/WebFiori/File/FileUploader#__construct).
  * Specify upload directory and allowed file types.
  * Specify the name of the input element that represents file upload input.
  * Call the method [`FileUploader::upload()`](https://webfiori.com/docs/WebFiori/File/FileUploader#upload) to initiate upload process.
...
Partial Backend Code (Shows FileUploader initialization only):
``` php 
...
class UploadView extends WebPage {
    public function __construct(){
        ...
        // is passed to the method FileUploader::setAssociatedFileName()
        $filesInput->setName('files-input');
...
The code which is used to upload multiple files at one call is the same as the one which is used to upload one file. The difference is that we have to set the attribute 'multiple' of the file input element and set the name to a syntax which looks like an array as following:
...
The method [`FileUploader::upload()`](https://webfiori.com/docs/WebFiori/File/FileUploader#upload) will return an associative array that contains upload status of uploaded files. In addition to getting files information as an associative array, it is possible to get uploaded files information as objects. The method [`FileUploader::uploadAsFileObj()`](https://webfiori.com/docs/WebFiori/File/FileUploader#uploadAsFileObj) will upload the files and return an array that contains objects of type [`UploadedFile`](https://webfiori.com/docs/WebFiori/File/UploadedFile). Also, it is possible to get uploaded files information even after the upload process is finished using the method [`FileUploader::getFiles()`](https://webfiori.com/docs/WebFiori/File/FileUploader#getFiles).


## Related Articles

* [Themes](learn/themes) - Handle theme asset uploads
* [Sending Emails](learn/sending-emails) - Attach uploaded files to emails
* [Web Services](learn/web-services) - Create file upload APIs
* [Routing](learn/routing) - Handle file upload routes
* [The Class Response](learn/class-response) - Send file upload responses
