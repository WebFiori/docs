...
This approach can be used to create a single directory with a single variable. What if we would like to create multiple directories and variables? To achieve this, the developer can use the method [`Lang::createAndSet()`](https://webfiori.com/docs/WebFiori/Framework/Lang#createAndSet).
...
You can also programmatically set the language for a page:
...
class HomePage extends WebPage {
    ...
    public function createHTMLNode() {
        $welcomeMsg = $this->get('pages/home/welcome-message', 'Welcome!');
        $this->insert('p')->text($welcomeMsg);
    }
}
```
...
class SampleService extends AbstractWebService {
    ...
    public function processRequest() {
        $this->sendResponse(Lang::getLabel('apis/say-hi'));
    }
}
```
...
Here are some additional examples for working with language variables:

``` php
// Get language variable with fallback
$message = Lang::getLabel('pages/contact/form-title', 'Contact Us');

// Check if a language variable exists
if (Lang::hasLabel('pages/special/title')) {
    $title = Lang::getLabel('pages/special/title');
}

// Get all variables in a directory
$homeVars = Lang::getLabels('pages/home');
...
Lang::loadTranslation($userLang);
```

### Language File Organization

For better organization, you can structure your language variables hierarchically:
...
class LangEN extends Lang {
    public function __construct() {
        ...
        $this->createAndSet('forms', [
            'validation' => [
                'required' => 'This field is required',
                'email' => 'Please enter a valid email',
                'min-length' => 'Minimum length is {length} characters'
            ],
            'buttons' => [
                'submit' => 'Submit',
                'cancel' => 'Cancel',
...
* [Web Pages](learn/web-pages) - Internationalize web page content
* [UI Package](learn/ui-package) - Create multilingual UI components
* [Global Constants](learn/env-vars) - Configure language settings
* [Themes](learn/themes) - Create multilingual themes
* [Background Tasks](learn/background-tasks) - Process translations asynchronously
