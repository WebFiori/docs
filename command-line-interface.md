...
* [Implementing CLI Commands](#implementing-cli-commands)
  * [Extending The Class "Command"](#extending-the-class-command)
...
$ php webfiori help --command-name=cron                
    ...
    Supported Arguments:
                      ...
                      --check:[Optional] Run a check against all jobs to check if it is time to execute them or not.
...
* Creating new PHP class that extends the class [`Command`](https://webfiori.com/docs/WebFiori/Cli/Command)
...
The first step in creating new command is to create new PHP class and make the class a child of the class [`Command`](https://webfiori.com/docs/WebFiori/Cli/Command). It can be noticed that the constructor of the class takes 3 parameters. The first one is the name of the command, the second one is an array of arguments and the last one is a description of the command. 
...
```
The method [`Command::getInput()`](https://webfiori.com/docs/WebFiori/Cli/Command#getInput) is used to read user input from an [`InputStream`](https://webfiori.com/docs/WebFiori/Cli/InputStream). The default input stream is represented by the class [`StdIn`](https://webfiori.com/docs/WebFiori/Cli/StdIn). The method accepts 3 parameters but for now, only the first one will be used. The first parameter of the method is a prompt text that will be shown to the user. The text is used to specify what the developer would like to get from the user.

The method [`Command::println()`](https://webfiori.com/docs/WebFiori/Cli/Command#println) is used to show output. It will send it directly to [`OutputStream`](https://webfiori.com/docs/WebFiori/Cli/OutputStream). The default output stream is represented by the class [`StdOut`](https://webfiori.com/docs/WebFiori/Cli/StdOut).
...
* Supply the arguments to the constructor of the class [`Command`](https://webfiori.com/docs/WebFiori/Cli/Command#__construct).
* Using the method [`Command::addArgs()`](https://webfiori.com/docs/WebFiori/Cli/Command#addArgs) to add multiple args at once.
* Using the method [`Command::addArg()`](https://webfiori.com/docs/WebFiori/Cli/Command#addArg) to add one argument.
* Using the method [`Command::addArgument()`](https://webfiori.com/docs/WebFiori/Cli/Command#addArgument) to add one argument as object of type [`CommandArgument`](https://webfiori.com/docs/WebFiori/Cli/CommandArgument).
...
It can be noticed that the statement which was shown by the method [`Command::warning()`](https://webfiori.com/docs/WebFiori/Cli/Command#warning) is pre-formatted. It is one of the methods which is used to format the output automatically.
...
One of the methods which is used to read user input is the method [`Command::getInput()`](https://webfiori.com/docs/WebFiori/Cli/Command#getInput). The framework provides the developers with the most commonly used helper inputs like asking a question or asking the user to select one of multiple options. 

### Prompt

Prompt is used to read any type of user input. The method [`Command::getInput()`](https://webfiori.com/docs/WebFiori/Cli/Command#getInput) is used to perform that task. The method accepts 3 parameters. The first one is the text that will be shown to the user. The second one is a default value which will be used when the user hit "Enter" without typing anything. The last one is a closure (a function) which can be used to validate user input.
...
The following code snippets uses the method `Command::println()` to show the output. 
...
The class [`Command`](https://webfiori.com/docs/WebFiori/Cli/Command) provides the developer with methods which can be used to display the output formatted without using formatting options. The methods can be used to show output for repeated tasks. The methods are:
* [`Command::error()`](https://webfiori.com/docs/WebFiori/Cli/Command#error): Output a message that represents an error.
* [`Command::info()`](https://webfiori.com/docs/WebFiori/Cli/Command#info): Output a message that represents extra information().
* [`Command::success()`](https://webfiori.com/docs/WebFiori/Cli/Command#success): Output a message that represents success state.
* [`Command::warning()`](https://webfiori.com/docs/WebFiori/Cli/Command#warning): Output a message that represents a warning.
