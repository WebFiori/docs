...
* Creating new PHP class inside theme directory that extends the class [`Theme`](https://webfiori.com/docs/webfiori/entity/Theme).
...
Themes in WebFiori Framework exist inside the folder `/themes`. The first step in creating new theme is to create new folder for it iside themes folder. let's assume that the name of the folder is 'custom-theme'. After creating theme folder, we can create 3 additional optional folders inside the new folder. The 3 folders will be used to hold theme CSS, JavaScript and images. Let's give the names `js`, `css` and `images`. This means that the folder structure of the theme will be as follows:
* `/themes/custom-theme/css` For CSS files.
* `/themes/custom-theme/js` For JavaScript files.
* `/themes/custom-theme/images` For the images that the theme might use.
...
* Implementing the abstract methods of the class [`Theme`](https://webfiori.com/webfiori/entity/Theme).

The name of the theme is needed because it acts like an identifier for it and is used to load it. The name can be set using the method [`Theme::setName()`](https://webfiori.com/webfiori/entity/Theme#setName). Setting the names of theme resources folders will help in loading all JavaScript and CSS files automatically. For JavaScript files, there exist the method [`Theme::setJsDirName()`](https://webfiori.com/webfiori/entity/Theme#setJsDirName), for CSS files, there exist the method [`Theme::setCssDirName()`](https://webfiori.com/webfiori/entity/Theme#setCssDirName) and for images, there exist the method [`Theme::setImagesDirName()`](https://webfiori.com/webfiori/entity/Theme#setImagesDirName). The last step is simply to define the actual structure of the page when the theme is loaded. 
...
* [`Theme::getHeadNode()`](https://webfiori.com/webfiori/entity/Theme#getHeadNode)
* [`Theme::getHeadrNode()`](https://webfiori.com/webfiori/entity/Theme#getHeadrNode)
* [`Theme::getAsideNode()`](https://webfiori.com/webfiori/entity/Theme#getAsideNode)
* [`Theme::getFooterNode()`](https://webfiori.com/webfiori/entity/Theme#getFooterNode)

Each method of the mentioned must return an instance of [`HTMLNode`](https://webfiori.com/docs/phpStructs/html/HTMLNode) except for the method [`Theme::getHeadNode()`](https://webfiori.com/webfiori/entity/Theme#getHeadNode). It must return an instance of [`HeadNode`](https://webfiori.com/docs/phpStructs/html/HeadNode). 
